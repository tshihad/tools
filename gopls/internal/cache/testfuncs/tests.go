...
func (b *indexBuilder) findTableDrivenSubtests(parent gobTest, param types.Object, rangeStmt *ast.RangeStmt, file *parsego.File, files []*parsego.File, info *types.Info) []gobTest {
	...
	tableEntries := b.extractTableTestCases(rangeStmt.X, files, info, file)
	if len(tableEntries) == 0 {
		return nil
	}

	// Find t.Run calls in the range body to confirm this is a table-driven test
	hasRun := false
	for _, stmt := range rangeStmt.Body.List {
		...
		// Check if first argument is a field access like tt.name
		if !b.isLoopVarFieldAccess(call.Args[0], loopVar, info) {
			continue
		}
...
// isLoopVarFieldAccess checks if expr is a field access on the loop variable
// (e.g., tt.name where tt is the loop variable).
func (b *indexBuilder) isLoopVarFieldAccess(expr ast.Expr, loopVar types.Object, info *types.Info) bool {
	...
	return info.ObjectOf(ident) == loopVar
}
...
func (b *indexBuilder) extractTableTestCases(expr ast.Expr, files []*parsego.File, info *types.Info, file *parsego.File) []tableTestCase {
	// Unwrap parentheses
	for {
	...
	for _, elt := range comp.Elts {
		...
		// Look for the "name" field - handle both keyed and unkeyed literals
